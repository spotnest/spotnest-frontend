    ...
    return (
        <aside className="flex h-full w-[280px] shrink-0 flex-col border-r border-[#e1e3e4] bg-white px-5 py-6 shadow-xs">
            <Link href="/" className="flex items-center gap-3 px-2" onClick={onNavigate}>
                <span className="grid h-10 w-10 place-items-center rounded-xl bg-[#00696b] text-base font-bold text-white shadow-xs">S</span>
                <div>
            ...
            <div className="mt-8 px-2 text-[11px] font-bold uppercase tracking-[0.16em] text-[#75777e]">Navigation</div>
            <nav className="mt-3 flex-1 space-y-1.5" aria-label="Dashboard navigation">
                {items.map((item) => {
                    const isBase = item.href.split("#")[0];
                    const isActive = isBase === pathname || item.children?.some((child) => pathname === child.href);

                    return (
                        <div key={item.label}>
                            <Link href={item.href} onClick={onNavigate} className={`flex items-center gap-3.5 rounded-xl px-3.5 py-3 text-sm font-semibold transition ${isActive ? "bg-[#d9f4f3] text-[#00696b]" : "text-[#44474d] hover:bg-[#f3f4f5] hover:text-[#191c1d]"}`}>
            ...
            <div className="rounded-2xl border border-[#e1e3e4] bg-[#f8f9fa] p-4 text-xs">
                ...
                <Link href="/contact" onClick={onNavigate} className="mt-2.5 inline-flex items-center gap-1 font-bold text-[#00696b] hover:text-[#004f51]">
