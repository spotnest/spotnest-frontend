...
export default function Sidebar({ onNavigate }: { onNavigate?: () => void }) {
    ...
    const baseNavigation = [
        { label: "Dashboard", href: dashboardHref, icon: "grid" as IconName },
        { label: "Users", href: "/users", icon: "users" as IconName, adminOnly: true },
        { label: "Properties", href: "/properties", icon: "home" as IconName },
        { label: "Requests", href: "/bookings", icon: "inbox" as IconName },
        { label: "Reports", href: `${dashboardHref}#reports`, icon: "chart" as IconName, adminOnly: true },
        { label: "Settings", href: "/settings", icon: "settings" as IconName },
    ];

    const navItems = baseNavigation.filter(item => !item.adminOnly || role === "admin");
    ...
    return (
        <aside className="flex h-full w-[250px] shrink-0 flex-col border-r border-[#e1e3e4] bg-white px-4 py-5">
            ...
            <div className="mt-8 px-3 text-[11px] font-bold uppercase tracking-[0.18em] text-[#75777e]">Workspace</div>
            <nav className="mt-3 space-y-1" aria-label="Main navigation">
                {navItems.map((item) => {
                    const routePath = item.href.split("#")[0];
                    const isActive = routePath === dashboardHref || routePath === "/dashboard"
                        ? pathname === "/dashboard" || pathname === "/admin/dashboard" || pathname === "/owner/dashboard" || pathname === "/user/dashboard"
            ...
            <div className="mt-auto space-y-3">
                <button
                    ...
                    className="flex w-full items-center gap-3 rounded-xl border border-red-200 bg-red-50/50 px-3 py-2.5 text-sm font-semibold text-red-600 transition hover:bg-red-100 hover:text-red-700 disabled:opacity-50"
                >
                ...
                <div className="rounded-2xl bg-[#eef6f5] p-4">
                    ...
                    <p className="mt-2 text-sm font-semibold text-[#191c1d]">Need a hand?</p>
